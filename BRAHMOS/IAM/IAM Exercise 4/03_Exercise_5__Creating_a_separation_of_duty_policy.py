"""
Interactive Guide for: Exercise 5 – Creating a separation of duty policy
"""
import time
import os
import re
...
def main():
    print("=== STARTING EXERCISE: Exercise 5 – Creating a separation of duty policy ===")
    web_bot = WebBot()
    print("\n--- INSTRUCTIONS ---")
    print_step("1. On the")
    ...
    print_step("3. Create the policy with the following information:")
    web_bot.try_action("3. Create the policy with the following information:")
    print_step("4. In the Policy Rules section, click")
    web_bot.try_action("4. In the Policy Rules section, click")
    print_step("5. Click")
    web_bot.try_action("5. Click")
    print_step("6. Search for roles in the")
    web_bot.try_action("6. Search for roles in the")
    print_step("7. After you select the")
    web_bot.try_action("7. After you select the")
    print_step("8. Click")
    web_bot.try_action("8. Click")
    print_step("9. Under")
    web_bot.try_action("9. Under")
    print_step("10. Click")
    web_bot.try_action("10. Click")
    print_step("11. Log out")
    web_bot.try_action("11. Log out")
    print("\n--- DATA ENTRY REQUIRED ---")
    val = get_input("these roles:", "A  sset Handling and Disposition,")
    val = get_input("Policy Name", "ABCs of Finance")
    val = get_input("Description", "Finance rules to maintain separation of duties")
    val = get_input("Business Unit", "Finance")
    val = get_input("Description of separation", "Finance department ABCs")
    val = get_input("", "Asset Handling and Disposition")
    val = get_input("", "Booking and Ledgers")
    val = get_input("", "Comparison and Review")
    val = get_input("one", "role.")
    val = get_input("Sue Thomas ,", "select")
    val = get_input("Sue, as the", "manager")
