"""
Interactive Guide for: Exercise 11 – Creating a provisioning policy using the service
"""
import time
import os
import re
...
def main():
    print("=== STARTING EXERCISE: Exercise 11 – Creating a provisioning policy using the service ===")
    web_bot = WebBot()
    print("\n--- INSTRUCTIONS ---")
    ...
    print_step("3. Preview")
    web_bot.try_action("3. Preview")
    print_step("4. Submit")
    web_bot.try_action("4. Submit")
    print_step("5. Click")
    web_bot.try_action("5. Click")
    print("\n--- DATA ENTRY REQUIRED ---")
    val = get_input("Policy Name", "M-Z Linux Accounts")
    ...
    val = get_input("Members", "All users in the organization")
    val = get_input("Entitlements", "Click")
    val = get_input("Provisioning options:", "Automatic")
    val = get_input("Target type:", "Service Selection Policy")
    val = get_input("Service type:", "POSIX Linux profile")
    val = get_input("Governing service selection policy name:", "Linux Service based on")
    val = get_input("Workflow:", "[Leave blank]")
    val = get_input("Entitlement Parameters", "[none set]")
    val = get_input("names starting with M through Z. Click", "Close.")
