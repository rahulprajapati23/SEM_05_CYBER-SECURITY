"""
Interactive Guide for: Exercise 4 – Verifying the password policy
"""
import time
import os
import re
...
def main():
    print("=== STARTING EXERCISE: Exercise 4 – Verifying the password policy ===")
    web_bot = WebBot()
    print("\n--- INSTRUCTIONS ---")
    print_step("1. On the")
    ...
    print_step("3. Click the small")
    web_bot.try_action("3. Click the small")
    print_step("4. Enter a password of")
    web_bot.try_action("4. Enter a password of")
    print_step("5. Read the error message on the screen.")
    web_bot.try_action("5. Read the error message on the screen.")
    print_step("6. Click the small triangle to the left of")
    web_bot.try_action("6. Click the small triangle to the left of")
    print("\n--- DATA ENTRY REQUIRED ---")
    val = get_input("Linux Service", "because the password is too short.")
    val = get_input("length for a password is four characters. Click", "Cancel.")
