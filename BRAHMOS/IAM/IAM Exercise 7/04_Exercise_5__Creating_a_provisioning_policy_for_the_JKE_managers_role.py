"""
Interactive Guide for: Exercise 5 – Creating a provisioning policy for the JKE managers role
"""
import time
import os
import re
...
def main():
    print("=== STARTING EXERCISE: Exercise 5 – Creating a provisioning policy for the JKE managers role ===")
    web_bot = WebBot()
    print("\n--- INSTRUCTIONS ---")
    print_step("1. On the")
    ...
    print_step("3. Create")
    web_bot.try_action("3. Create")
    print_step("4. Preview")
    web_bot.try_action("4. Preview")
    print_step("5. Close")
    web_bot.try_action("5. Close")
    print_step("6. Submit")
    web_bot.try_action("6. Submit")
    print("\n--- DATA ENTRY REQUIRED ---")
    val = get_input("Note", ":  This provisioning policy takes precedence over Admin Linux Accounts because it has a")
    val = get_input("Policy name", "Manager Linux Accounts")
    val = get_input("Policy Status", "Enable")
    val = get_input("Priority", "50")
    val = get_input("Members (Section)", "Select :")
    val = get_input("Add organizational role", "JKE Managers")
    ...
    val = get_input("Change UNIX shell value to", "/bin/ksh")
