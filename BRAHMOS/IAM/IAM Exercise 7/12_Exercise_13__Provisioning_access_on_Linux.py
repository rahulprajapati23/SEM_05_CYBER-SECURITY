"""
Interactive Guide for: Exercise 13 – Provisioning access on Linux
"""
import time
import os
import re
...
def main():
    print("=== STARTING EXERCISE: Exercise 13 – Provisioning access on Linux ===")
    web_bot = WebBot()
    print("\n--- INSTRUCTIONS ---")
    print_step("1. On the")
    web_bot.try_action("1. On the")
    print_step("2. Click the small arrow to the right of")
    web_bot.try_action("2. Click the small arrow to the right of")
    print_step("3. Click the group name")
    web_bot.try_action("3. Click the group name")
    print_step("4. On the")
    ...
    print_step("8. Open Firefox. Enter the URL -")
    web_bot.try_action("8. Open Firefox. Enter the URL -")
    print_step("9. Log in as user")
    web_bot.try_action("9. Log in as user")
    print_step("10. Click")
    web_bot.try_action("10. Click")
    print_step("11. Click")
    web_bot.try_action("11. Click")
    print_step("12. To confirm successful provisioning, open a")
    web_bot.try_action("12. To confirm successful provisioning, open a")
    print_step("13. Log out")
    web_bot.try_action("13. Log out")
    print_step("14. Click")
    web_bot.try_action("14. Click")
    print_step("15. Logout")
    web_bot.try_action("15. Logout")
    print("\n--- DATA ENTRY REQUIRED ---")
    val = get_input("Application", ". Select a workflow of")
    val = get_input("Note", ":")
    val = get_input("John is", "not a member")
