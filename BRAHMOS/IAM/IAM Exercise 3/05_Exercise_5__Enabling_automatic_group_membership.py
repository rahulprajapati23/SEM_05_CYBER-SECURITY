"""
Interactive Guide for: Exercise 5 – Enabling automatic group membership
"""
import time
import os
import re
...
def main():
    print("=== STARTING EXERCISE: Exercise 5 – Enabling automatic group membership ===")
    web_bot = WebBot()
    print("\n--- INSTRUCTIONS ---")
    print_step("1. Log in to the IBM Security Identity Manager Administrative Console with")
    web_bot.try_action("1. Log in to the IBM Security Identity Manager Administrative Console with")
    print_step("2. On the")
    web_bot.try_action("2. On the")
    print_step("3. In the Group Settings section, enable")
    web_bot.try_action("3. In the Group Settings section, enable")
    print_step("4. Click")
    web_bot.try_action("4. Click")
    print("\n--- DATA ENTRY REQUIRED ---")
    val = get_input("In the last exercise, you added your ID to the", "System")
    val = get_input("IBM Security Identity Manager has a feature that automatically populates the", "Manager")
