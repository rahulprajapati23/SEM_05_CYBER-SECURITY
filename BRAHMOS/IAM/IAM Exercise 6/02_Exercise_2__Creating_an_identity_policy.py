"""
Interactive Guide for: Exercise 2 – Creating an identity policy
"""
import time
import os
import re
...
def main():
    print("=== STARTING EXERCISE: Exercise 2 – Creating an identity policy ===")
    web_bot = WebBot()
    print("\n--- INSTRUCTIONS ---")
    print_step("1. On the")
    ...
    print_step("3. In the Rule section, select the first attribute to be")
    web_bot.try_action("3. In the Rule section, select the first attribute to be")
    print_step("6 and set Apply case to")
    web_bot.try_action("6 and set Apply case to")
    print_step("4. Click")
    web_bot.try_action("4. Click")
    print_step("5. Click")
    web_bot.try_action("5. Click")
    print("\n--- DATA ENTRY REQUIRED ---")
    val = get_input("Name", "Linux Identity Policy")
    val = get_input("Description", "Identity Policy for Linux Service")
    val = get_input("Status", "Enabled")
    val = get_input("User type", "Person")
    val = get_input("Make policy available to", "This business unit and its subunits")
    val = get_input("Business unit", "JK Enterprises")
    val = get_input("Targets  (Section)", "Click")
    val = get_input("Hint", ":  After creating a rule in")
