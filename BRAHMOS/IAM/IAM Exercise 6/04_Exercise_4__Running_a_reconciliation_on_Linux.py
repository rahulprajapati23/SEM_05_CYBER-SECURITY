"""
Interactive Guide for: Exercise 4 – Running a reconciliation on Linux
"""
import time
import os
import re
...
def main():
    print("=== STARTING EXERCISE: Exercise 4 – Running a reconciliation on Linux ===")
    web_bot = WebBot()
    print("\n--- INSTRUCTIONS ---")
    print_step("1. On the")
    ...
    print_step("3. A reconciliation schedule is automatically created by the")
    web_bot.try_action("3. A reconciliation schedule is automatically created by the")
    print_step("4. Modify the schedule for this reconciliation to run")
    web_bot.try_action("4. Modify the schedule for this reconciliation to run")
    print_step("5. Click")
    web_bot.try_action("5. Click")
    print_step("1. Return to")
    web_bot.try_action("1. Return to")
    print_step("2. Click the small")
    web_bot.try_action("2. Click the small")
    print_step("3. View the")
    web_bot.try_action("3. View the")
    print_step("4. Close")
    web_bot.try_action("4. Close")
    print_step("1. Return to")
    web_bot.try_action("1. Return to")
    print_step("2. Click the small")
    web_bot.try_action("2. Click the small")
    ...
    web_bot.try_action("4. Close")
    print("\n--- DATA ENTRY REQUIRED ---")
    val = get_input("Reconciliation", "Schedule")
    val = get_input("when prompted, and", "submit")
