"""
Interactive Guide for: Exercise 3 – Creating a password policy
"""
import time
import os
import re
...
def main():
    print("=== STARTING EXERCISE: Exercise 3 – Creating a password policy ===")
    web_bot = WebBot()
    print("\n--- INSTRUCTIONS ---")
    print_step("1. On the")
    web_bot.try_action("1. On the")
    print_step("2. Create a new password policy with the following information.")
    web_bot.try_action("2. Create a new password policy with the following information.")
    print_step("3. Click")
    web_bot.try_action("3. Click")
    print("\n--- DATA ENTRY REQUIRED ---")
    val = get_input("least", "four")
    val = get_input("Name", "Linux Password Policy")
    val = get_input("Description", "Password policy for Linux Service")
    ...
    val = get_input("Rules (Section)", "Minimum length")
