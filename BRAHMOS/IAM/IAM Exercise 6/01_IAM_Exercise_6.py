"""
Interactive Guide for: IAM Exercise 6
"""
import time
import os
import re
from selenium import webdriver
from selenium.webdriver.common.by import By
from selenium.webdriver.common.keys import Keys

def print_step(msg):
    print(f"\n[STEP] {msg}")

def get_input(field, suggested):
    print(f"\n[INPUT] Field: {field}")
    val = input(f"Enter value (Suggested: {suggested}): ")
    return val if val.strip() else suggested

def run_command(desc, auto_cmd=None):
    print(f"\n[ACTION] Command required for: {desc}")
    if auto_cmd:
        print(f"Auto-executing known command: {auto_cmd}")
        try:
             os.system(auto_cmd)
             print("Execution finished.")
        except Exception as e:
             print(f"Error executing: {e}")
    else:
        # User requested NO PROMPTS. Just display the instruction.
        print("(No automated command matched. Please execute manually if required.)")

class WebBot:
    def __init__(self):
        print("Initializing Browser for Web Automation (Firefox)...")
        try:
            self.driver = webdriver.Firefox() # Assumes geckodriver is in PATH
        except Exception as e:
            print(f"Warning: Firefox driver (geckodriver) not found. Web automation skipped. Error: {e}")
            self.driver = None

    def try_action(self, step_text):
        if not self.driver: return
        step_lower = step_text.lower()
        try:
            if "log in" in step_lower or "login" in step_lower:
                 print("Attempting to find Login fields...")
                 # Generic login attempt
                 try:
                     self.driver.find_element(By.NAME, "j_username").send_keys("system")
                     self.driver.find_element(By.NAME, "j_password").send_keys("secret")
                 except: pass
            elif "click" in step_lower:
                 # Extract quoted text or prominent words
                 match = re.search(r"Click\s+([A-Za-z0-9\s]+)", step_text, re.IGNORECASE)
                 target = match.group(1).strip() if match else None
                 if target:
                     print(f"Attempting to Click: {target}")
                     # Try generic xpath by text
                     el = self.driver.find_element(By.XPATH, f"//*[contains(text(), '{target}')]")
                     el.click()
            elif "enter" in step_lower or "type" in step_lower:
                 # Input attempt
                 pass
        except Exception as e:
            print(f"Web Action Failed (Expected, as this is heuristic): {e}")

def main():
    print("=== STARTING EXERCISE: IAM Exercise 6 ===")
    web_bot = WebBot()
    print("\n--- INSTRUCTIONS ---")
    print_step("1.      Log in to the IBM Security Identity Manager Administrative Console as the system administrator with")
    web_bot.try_action("1.      Log in to the IBM Security Identity Manager Administrative Console as the system administrator with")
    print_step("2.      On the Home tab, navigate to                               Manage Services.")
    web_bot.try_action("2.      On the Home tab, navigate to                               Manage Services.")
    print_step("3.      Click      Create        .")
    web_bot.try_action("3.      Click      Create        .")
    print_step("4.      Wait for the list of services to appear. Ensure that Business unit is set to                                                                 JK Enterprises.")
    web_bot.try_action("4.      Wait for the list of services to appear. Ensure that Business unit is set to                                                                 JK Enterprises.")
    print_step("5.       Select        POSIX Linux profile                       and click         Next      .")
    web_bot.try_action("5.       Select        POSIX Linux profile                       and click         Next      .")
    print_step("6.      Use the information in the following table to complete the Create a Service form (Keep other settings")
    web_bot.try_action("6.      Use the information in the following table to complete the Create a Service form (Keep other settings")
    print_step("7. Click")
    web_bot.try_action("7. Click")
    print_step("8. Verify that the Linux Service was added.")
    web_bot.try_action("8. Verify that the Linux Service was added.")
    print("\n--- DATA ENTRY REQUIRED ---")
    val = get_input("Service name", "Linux Service")
    val = get_input("Description", "Linux Service on ISIM")
    val = get_input("Tivoli Directory Integrator location", "rmi://isim.test:1099/ITDIDispatcher")
    val = get_input("Managed resource location", "isim.test")
    val = get_input("Owner", "Bob Smith")
    val = get_input("Service Prerequisite", "[Leave blank]")
    val = get_input("Use Shadow File (Additional", "Checked")
    val = get_input("Command used to query failed logins", "pam_tally2")
    val = get_input("Administrator name (Authentication", "root")
    val = get_input("Is Sudo User? (Authentication Section)", "Checked")
    val = get_input("Password(Authentication Section)", "P@ssw0rd")
    val = get_input("Configure Policy(Section)", "Yes, create a policy for manually requesting account")
    val = get_input("Perform a supporting data", "[Leave cleared]")
    val = get_input("Hint", ":")
    val = get_input("section of the", "Create Service")
    val = get_input("Integrator Adapter service is", "running")
    print("\n=== EXERCISE COMPLETED SUCCESSFULLY === ")

if __name__ == "__main__":
    main()
